...
def obtenerIntento(letrasProbadas):
	...
	while True:
		...
		if len(intento) != 1:
			...
			return intento 
def jugarDeNuevo(): 
	"""Esta función devuelve True si el jugador quiere volver a jugar, encaso contrario devuelve False."""
...
while True:
	...
	if intento in palabraSecreta:
		...
		if len(letrasIncorrectas) == len(IMÁGENES_AHORCADO) - 1:
			...
			juegoTerminado = True
			"""Preguntar al jugador si quiere volver a jugar (pero sólo si el juego
			ha terminado)."""
	if juegoTerminado:
		if jugarDeNuevo():
			letrasIncorrectas = ''
